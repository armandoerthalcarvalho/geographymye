...
const STORAGE_KEY = 'geography-midyear-quiz-progress-v1';
const bank = window.GEOGRAPHY_MIDYEAR_QUESTION_BANK || { questions: [], metadata: {} };
...
function renderShortAnswer(question, record) {
  els.answerArea.innerHTML = `
    <textarea name="answer" maxlength="800" placeholder="Write a clear geography answer using evidence, cause/effect and precise vocabulary.">${escapeHtml(record?.answer || '')}</textarea>
  `;
}
...
function grade(question, answer) {
  ...
  const hasGeoTerm = /(europe|migration|climate|population|demograph|russia|ukraine|cold war|energy|border|vegetation|welfare|dependency|soviet|european union|geopolitic)/i.test(answer);
  const causeEffect = /(because|therefore|consequence|impact|leads to|causes|affects|due to|as a result)/i.test(answer);
  const enoughLength = answer.split(/\s+/).filter(Boolean).length >= 25;
  const raw = (hits.length / question.requiredKeywords.length) * 0.55 + (hasGeoTerm ? 0.2 : 0) + (causeEffect ? 0.1 : 0) + (enoughLength ? 0.15 : 0);
...
function renderFeedback(question, record) {
  ...
  const shortTip = question.type === 'short-answer'
    ? `<p><b>Auto-check:</b> This response earned ${Math.round(record.score * 100)}%. Add precise geography vocabulary, cause/effect and a clear Europe connection.</p>`
    : '';
...
function updateDiagnosis() {
  ...
  if (!items.length) {
    els.diagnosisList.innerHTML = '<p>Submit answers to unlock personalised priorities.</p>';
    return;
  }
...
function diagnosisTip(domain, missed) {
  if (domain.includes('political territory')) return `Review borders, Eurasia, transcontinental states and regional organizations. Missed: ${missed}.`;
  if (domain.includes('physical geography')) return `Drill climate types, climographs, vegetation and human activity links. Missed: ${missed}.`;
  if (domain.includes('Demography')) return `Revisit pyramids, dependency ratio, aging and demographic transition. Missed: ${missed}.`;
  if (domain.includes('Migration')) return `Separate push/pull factors, refugee routes and EU responsibility debates. Missed: ${missed}.`;
  if (domain.includes('Russia-Ukraine')) return `Connect language, territory, energy, security and refugee impacts. Missed: ${missed}.`;
  if (domain.includes('Cold War')) return `Review Iron Curtain, Soviet collapse and post-1991 border changes. Missed: ${missed}.`;
  return `Focus on EU energy, CAP, uneven development and policy fragmentation. Missed: ${missed}.`;
}
...
els.reset.addEventListener('click', () => {
  if (!confirm('Reset all saved Geography quiz progress?')) return;
